...
DEFAULT_OUTPUT_DIR = 'outputs/qaoa_unweighted/N12/'
...
def plot_p_star_vs_gap(input_csv: str, output_dir: str = None,
    ...
    for threshold in thresholds:
        ...
        if np.sum(reached_mask) > 2:
            x_valid = df[gap_col].values[reached_mask]
    ...
    # Desktop mirror copy
    try:
        _, desktop_dir = get_run_dirs("p_star_vs_gap", timestamp=True)
        save_file(output_file, "p_star_vs_gap", _desktop_dir=desktop_dir)
        save_run_info(desktop_dir, "p_star_vs_gap", extra_info={"input_csv": input_csv})
...
if __name__ == "__main__":
    ...
    if not os.path.exists(input_csv):
        ...
        print(f"     - outputs/qaoa_unweighted/N12/qaoa_sweep_N12_p1to10_filtered.csv")
        print(f"     - outputs/qaoa_unweighted/N12/qaoa_sweep_N12_p1to10.csv")
        sys.exit(1)
